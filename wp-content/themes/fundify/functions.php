...
function fundify_scripts() {
	...
	wp_register_script('fundify-scripts', get_template_directory_uri() . '/js/fundify.js');
	...
	wp_enqueue_script( 'magnific', false , array( 'jquery' ), '2.1.4', true );
	wp_enqueue_script( 'fundify-scripts', false, array( 'magnific', 'jquery-masonry' ), 20130522, true );
...
function fundify_embed_markup($html, $post_id) {
	$html = '<a href="share-widget" class="share-widget fancybox"><i class="icon-code"></i></a>';
	return $html;
}
add_filter('idsocial_embed_link_markup', 'fundify_embed_markup', 10, 2);

function fundify_swap_purchaseform($url, $project_id) {
	global $post;
	if (isset($post->ID) && $post->ID > 0 && $post->post_type == 'ignition_product') {
		$prefix = idf_get_querystring_prefix();
		$url = get_the_permalink().$prefix.'purchaseform=fundify&amp;prodid='.(isset($project_id) ? $project_id : '');
	}
	return $url;
}

add_filter('idcf_purchase_url', 'fundify_swap_purchaseform', 10, 2);

function idcf_campaign_notes( $post_id ) {
	// needs to be converted to work with IDCF
	$project_id = get_post_meta($post_id, 'ign_project_id', true);
	$project = new ID_Project($project_id);
	$end_date = date( get_option( 'date_format' ), strtotime( $project->end_date() ) );
...
//add_action( 'atcf_campaign_before', 'idcf_campaign_notes' );
