...
/*
	...
	var ancre = curUrl.split('#');
	var coms = ancre[1].split('-')

	if( coms[0] === 'comment' ){
		$('.sort-tabs.campaign').find('li').removeClass('active');
		$('.comments_tab_btn').addClass('active');
		$( '.campaign-tabs' ).children( 'div' ).hide();
		$( '.campaign-view-descrption' ).show();
		$( '.campaign-tabs' ).find('#comments').show();
	}
*/	
	/**
