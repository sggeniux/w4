...
<?php
    if ( $type == 'recurring' ) {
        ...
        _e('This transaction is a pre-order. You will be charged at a later time.', 'memberdeck');
    }
    else {
        if ($combined_product) {
            _e('This product is paired with a subscription that will not end until canceled.', 'memberdeck');
        }
        else {
            _e('This is a one time payment. You will be charged immediately.', 'memberdeck');
        }
    }
?>
</span>