		...
		if (empty($text)) {
			$message .= '<div style="padding:10px;background-color:#f2f2f2;">
							...
							<h2>'.$subject.'</h2>
								...
								<div style="margin:10px;">
									...
									<br>
									<a href="'.$durl.$prefix.'idc_renewal_checkout='.$level->id.'&price='.$level->renewal_price.'">'.__('Follow this link', 'memberdeck').'</a> '.__('to renew your product or use the address below', 'memberdeck').'.<br/>
									<a href="'.$durl.$prefix.'idc_renewal_checkout='.$level->id.'&price='.$level->renewal_price.'">'.$durl().$prefix.'idc_renewal_checkout='.$level->id.'&amp;price='.$level->renewal_price.'</a>
			...
			$merge_swap = array(
				...
				array(
					...
					'swap' => $durl.$prefix.'idc_renewal_checkout='.$level->id.'&price='.$level->renewal_price
				)
			);
