...
function id_submissionForm($post_id = null) {
	...
	if (!empty($post_id) && $post_id > 0) {
		...
		//$company_name = (isset($creator_info['name']) ? $creator_info['name'] : '');
		$company_logo = (isset($creator_info['logo']) ? $creator_info['logo'] : '');
		//$company_location = (isset($creator_info['location']) ? $creator_info['location'] : '');
		//$company_url = (isset($creator_info['url']) ? $creator_info['url'] : '');
		//$company_fb = (isset($creator_info['facebook']) ? $creator_info['facebook'] : '');
		//$company_twitter = (isset($creator_info['twitter']) ? $creator_info['twitter'] : '');
...
function ide_backer_profile_tab() {
	...
	echo '<li '.(isset($profile) && $profile == $user_id ? 'class="active"' : '').'><a href="'.md_get_durl().$prefix.apply_filters('idc_backer_profile_slug', 'backer_profile').'='.$user_id.'">'.__('Backer Profile', 'ignitiondeck').'</a></li>';
}
...
function ide_creator_profile_tab() {
	...
	if (current_user_can('create_edit_projects')) {
		...
		echo '<li '.(isset($profile) && $profile == $user_id ? 'class="active"' : '').'><a href="'.md_get_durl().$prefix.apply_filters('idc_creator_profile_slug', 'creator_profile').'='.$user_id.'">'.__('Creator Profile', 'ignitiondeck').'</a></li>';
	}
}
